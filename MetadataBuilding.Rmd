...
dir.create(paste(dataAnPath, diversityDir, sep = ""),
           showWarnings = TRUE,
           recursive    = FALSE)
diversityPath <- paste(dataAnPath, diversityDir, sep = "")


i <- 1
for(i in 1:nrow(metadataMasterNew)) {
  temp <- read.table(paste(metadataMasterNew$FilePath[i], metadataMasterNew$meta[i], sep = ""),
                      sep = "\t", header = TRUE, dec = ".", comment.char = "", quote = "", stringsAsFactors = TRUE,
                      as.is = TRUE)
 
  temp$row.names <- NULL

  trans <-  read.table(paste(metadataMasterNew$taxPath[i], metadataMasterNew$taxName[i], sep = ""),
                      sep = "\t", header = TRUE, dec = ".", comment.char = "", quote = "", stringsAsFactors = TRUE,
  ...
  write.table(temp2, file=paste(dataAnPath, "diversityDir/", metadataMasterTax$ShortName[i], ".meta.div.tsv", sep = ""),
              append    = FALSE,
              sep       = "\t",
              row.names = FALSE,
              quote=FALSE)
...
combine runs of sequencing by organism

comColNames <- c("Primer", "ShortName", "DataAnFilePath", "meta", "table", "taxPath", "taxName")

combinedTableITS <- subset(metadataMasterTax, 
                           metadataMasterTax$Primer == "its1f" | metadataMasterTax$Primer == "its2", 
                           select = c(comColNames))
row.names(combinedTableITS) <- NULL
combinedTableITS$tablePath  <- paste(combinedTableITS$DataAnFilePath, combinedTableITS$table, sep = "")
combinedTableITS$metaPath   <- paste(combinedTableITS$DataAnFilePath, combinedTableITS$meta, sep = "")
combinedTableITS$taxFillPath  <- paste(combinedTableITS$taxPath, combinedTableITS$taxName, sep = "")

combinedTableOm <- subset(metadataMasterTax, 
                          metadataMasterTax$Primer == "omlo" | metadataMasterTax$Primer == "omup", 
                          select = c(comColNames))
row.names(combinedTableOm) <- NULL
combinedTableOm$tablePath  <- paste(combinedTableOm$DataAnFilePath, combinedTableOm$table, sep = "")
combinedTableOm$metaPath   <- paste(combinedTableOm$DataAnFilePath, combinedTableOm$meta, sep = "")
combinedTableOm$taxFillPath  <- paste(combinedTableOm$taxPath, combinedTableOm$taxName, sep = "")


combinedTablePhy <- subset(metadataMasterTax, 
                           metadataMasterTax$Primer == "phy", 
                           select = c(comColNames))
row.names(combinedTablePhy) <- NULL
combinedTablePhy$tablePath  <- paste(combinedTablePhy$DataAnFilePath, combinedTablePhy$table, sep = "")
combinedTablePhy$metaPath   <- paste(combinedTablePhy$DataAnFilePath, combinedTablePhy$meta, sep = "")
