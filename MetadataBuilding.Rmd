...
for(i in 1:nrow(metadataMasterTax)){
  temp <-  read.table(paste(metadataMasterTax$taxPath[7], metadataMasterTax$taxName[7], sep = ""),
                      sep = "\t", header = TRUE, dec = ".", comment.char = "", quote = "", stringsAsFactors = TRUE,
                      as.is = TRUE, check.names = FALSE, colClasses=c("row.names"="character", "taxonomy"="character"))
temp$row.names <- NULL  

png(filename=paste(pathDataAn, "otuRecap/", metadataMasterTax$ShortName[7], ".otuRecap.png", sep = ""), 
...
temp <-  read.table("/home/CFIA-ACIA/tremblaye/NGS_for_Fungi_Detection/dataAn/taxFillTables/its1f.run7.table.taxFill.tsv",
                      sep = "\t", header = TRUE, dec = ".", comment.char = "", quote = "", stringsAsFactors = TRUE,
                      as.is = TRUE, check.names = FALSE, colClasses=c("row.names"="character", "taxonomy"="character"))
temp$row.names <- NULL  

png(filename=paste(pathDataAn, "otuRecap/", "its1f.run7.otuRecap.png", sep = ""), 
      width = 800, height = 500, units = "px")
