...
###########
############
#this is rerunning the runs by organism and including runs 13-15 -woot woot
...
#from wen chen: meta_new <- meta[match(colnames(otu)[-ncol(otu)], rownames(meta)), ] does not work or help
 ...
 tabletemp$row.names <- NULL
  ...
  metatemp <-  read.table(paste(pathDataAn, "ITS.meta.tsv", sep = ""),
                      sep = "\t", header = TRUE, dec = ".", comment.char = "", quote = "", stringsAsFactors = TRUE,
                      as.is = TRUE)
metatempnew <- metatemp[match(colnames(tabletemp)[-ncol(tabletemp)], rownames(metatemp)), ]
View(metatempnew)
 
  temp2 <- OTU.diversity(tabletemp, metatemp)
temp2 <- OTU.diversity(list(data=tabletemp), meta=metatemp)
...
# Put this data into the png graph for the open filehandle:
  OTU.recap(list(data=temp), ranks=c("k", "p", "c", "o", "f", "g", "s"), brewer.pal="Set3")

# Close the png graph file handle:
  dev.off()

df <- OTU.recap(list(data=temp), ranks=c("k", "p", "c", "o", "f", "g", "s"), brewer.pal="Set3")
# Write out the dataframe:
...
df2 <-OTU.recap(list(data=temp), ranks=c("k", "p", "c", "o", "f", "g", "s"), brewer.pal="Set2")
...
# Put this data into the png graph for the open filehandle:
  OTU.recap(list(data=temp), ranks=c("k", "p", "c", "o", "f", "g", "s"), brewer.pal="Set1")

# Close the png graph file handle:
  dev.off()

df3 <-OTU.recap(list(data=temp), ranks=c("k", "p", "c", "o", "f", "g", "s"), brewer.pal="Set1")
...
metaTemp <- rbindlist(lapply(metaToAppend, fread, header = "auto", select = metaColToKeep,
                             showProgress = TRUE))
metaSuperITS <- metaTemp
write.table(metaTemp, file = paste(pathDataAn, "metaSuper.ITS.tsv", sep = ""),
            append = FALSE, sep = "\t", row.names = FALSE, quote = FALSE)

# For the Om set:
metaToAppend <- combinedTableOm$metaPath
metaTemp <- rbindlist(lapply(metaToAppend, fread, header = "auto", select = metaColToKeep,
                             showProgress = TRUE))
metaSuperOm <- metaTemp
write.table(metaTemp, file = paste(pathDataAn, "metaSuper.Om.tsv", sep = ""),
            append = FALSE, sep = "\t", row.names = FALSE, quote = FALSE)

# For the Phy set:
metaToAppend <- combinedTablePhy$metaPath
metaTemp <- rbindlist(lapply(metaToAppend, fread, header = "auto", select = metaColToKeep,
                             showProgress = TRUE))
metaSuperPhy <- metaTemp
write.table(metaTemp, file = paste(pathDataAn, "metaSuper.Phy.tsv", sep = ""),
            append = FALSE, sep = "\t", row.names = FALSE, quote = FALSE)
rm(metaTemp)
```
...
tablesList <- combinedTableITS$tablePath
tablesDF <- lapply(tablesList, fread.OTU)

View(tablesDF[[1]])
summary(tablesDF)
is.data.frame(tablesDF[[1]])
valid.OTU(tablesDF[[1]])
```

The following line doesn't work - none of the samples are the same across tables.
Emilie will try to combine the tables using chime
