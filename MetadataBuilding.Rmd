...
## Almost have it - tax.fill  works! now just need to loop through metadataMaster list!
i <- 1
for(i in 1:nrow(metadataMaster)){
  temp <-  read.table(paste(pathDataAn, metadataMaster$table[i], sep = ""),
                      sep = "\t", 
                      header = TRUE, dec = ".",
                      comment.char = "", quote = "", 
  ...
  write.table(temp, file = file.path(paste(pathDataAn, "taxFillTables/", sep = ""), 
              metadataMaster$ShortName, ".table.taxFill.tsv", sep = ""),
              append    = FALSE,
              sep       = "\t",
              col.names = NA)
  }
...
Ecrire ta table de meta (tracabilité man!)
```{r}
i = 1
for(i in 1:length(meta)){
