...
for(i in 1:nrow(metadataMasterTax)){
  temp <-  read.table(paste(metadataMasterTax$taxPath[1], metadataMasterTax$taxName[1], sep = ""),
                      sep = "\t", header = TRUE, dec = ".", comment.char = "", quote = "", stringsAsFactors = TRUE,
...
OTU.recap(data = list(data = temp), ranks=c("k", "p", "c", "o", "f", "g", "s"), brewer.pal="Greens")
dev.off()  
}
dim(temp)
summary(temp)
