...
ITS_phylRank <-get.rank(tabletemp, rank="p")

test1 <- group.abundance(list(data=tabletemp), rank="p")
dim(tabletemp)
otuIDs <- row.names(tabletemp)

# Open file-handle to get ready to make the png graph:
png(filename=paste(RanksPath, "ITS", "test.png", sep = ""),
      width = 1200, height = 1000, units = "px")
  
  
group.OTU(tabletemp, rank = "p", otuIDs ="otuIDs", meta = metatemp, meta.factor="TrapType", boxplot = False)
# Close the png graph file handle:
  dev.off() 
  
#test3 <- shared.OTU(list(data=tabletemp)) 
#test4 <- shared.Taxa(list(data=tabletemp))   
...
test5 <- tax.abund(tabletemp, otu2=NULL, rank = "p", drop.unclassified = TRUE, top = 20, count = FALSE, mode = "percent")
